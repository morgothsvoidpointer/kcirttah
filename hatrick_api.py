...
def get_match_data_chpp(chpp,match_id=36816805):
    print('getting data from match '+str(match_id))
    ...
    home_spex=[]
    player_count=0
    keeperobj=home_lineup['KEEPER'][100]
    if keeperobj is not None:
        did=keeperobj.id
    ...
    #individual_orders_home_final=[str(sum(x)) for x in zip(individual_orders_home,filled_slots_home)]
    individual_orders_home_final=[str(x) for x in individual_orders_home[:11]]
    home_data['Orders']=''.join(individual_orders_home_final)
    ...
    keeperobj=away_lineup['KEEPER'][100]
    if keeperobj is not None:
        did=keeperobj.id
    ...
    individual_orders_away_final=[str(x) for x in individual_orders_away[:11]]
    away_data['Orders']=''.join(individual_orders_away_final)   
...
#%%    
    ...
    away_data['Orders']=''.join([str(ioh) for ioh in individual_orders_away[:11]])
