...
from scipy.stats import poisson as poisson_sc
...
def power(number, power):
    ...
    if number == power == 0:
        ...
        print('zero to power zero returns error')
        return NA_ERROR
...
def poisson(X,mean,cumulative):
    #Excel reference: https://support.microsoft.com/en-gb/office/poisson-function-d81f7294-9d7c-4f75-bc23-80aa8624173a
    if not cumulative:
        #print(X)
        #print('calc poisson')
        if X==0:
            #print(poisson_sc.cdf(X,mean))
            return poisson_sc.cdf(X,mean)
        else:
            # print(poisson_sc.cdf(X,mean)-poisson_sc.cdf(X-1,mean))
            return poisson_sc.cdf(X,mean)-poisson_sc.cdf(X-1,mean)           
    else:
        #print('calc poisson')
        #print(X)
        #print(poisson_sc.cdf(X,mean))
...
def counta(vec):
    if isinstance(vec,int):
        ...
        print('found vector')
        print(vec)
        v=[i for i in vec if i is not None]
